...
+ (JSCommand*) command:(NSArray*)rnCommand dispatcher:(JSCommandDispatcher*)dispatcher module:(ConnectSDKModule*)module
{
    JSCommand* command = [JSCommand new];
    command.dispatcher = dispatcher;
    command.module = module;
    command.deviceId = [rnCommand objectAtIndex:0];
    command.args = [rnCommand objectAtIndex:4];
    command.isSubscription = [[rnCommand objectAtIndex:5] boolValue];
...
- (void) sendSuccessResult:(NSMutableArray*)result
{
    if (self.cancelled) return;

    if (self.isSubscription) {
        [self.module sendModuleResult:result callbackId:_commandId deviceId:_deviceId];
    }
    else {
        [self.module sendModuleResult:result deviceId:_deviceId];
        [self complete];
    }
}

- (void) sendFailureResult:(NSMutableArray*)result
{
    if (self.cancelled) return;

    NSMutableArray* args = [NSMutableArray arrayWithObject:@"error"];
    [args addObjectsFromArray:result];
    [_module sendModuleResult:args deviceId:_deviceId];
    [self complete];
...
- (MediaPlayerSuccessBlock) mediaLaunchSuccess
{
    return ^(MediaLaunchObject *mediaLaunchObject) {
        ...
        MediaControlWrapper* controlWrapper = [[MediaControlWrapper alloc] initWithModule:self.module
                                                                             mediaControl:mediaLaunchObject.mediaControl];
        [self.module addObjectWrapper:controlWrapper];
        ...
        if (mediaLaunchObject.playListControl) {
            PlaylistControlWrapper *playlistControlWrapper = [[PlaylistControlWrapper alloc]
                initWithModule:self.module
               playlistControl:mediaLaunchObject.playListControl];
            [self.module addObjectWrapper:playlistControlWrapper];
...
- (WebAppLaunchSuccessBlock) webAppLaunchSuccess
{
    return ^(WebAppSession* session) {
        ...
        JSObjectWrapper* wrapper = [[WebAppSessionWrapper alloc] initWithModule:self.module session:session];
        [self.module addObjectWrapper:wrapper];
...
{
    return ^(float value){
        [self sendSuccessWithArray:@[ [NSNumber numberWithInt:value * 100] ]];
    };
}
...
- (FailureBlock) failure
{
    return ^(NSError* error){
        if (self.cancelled) return;
        NSMutableArray* args = [NSMutableArray arrayWithObject:@"error"];
        [args addObject:[error localizedDescription]];
...
{
    JSCommand* command = [JSCommand command:rnCommand dispatcher:self module:module];
    command.commandId = [rnCommand objectAtIndex:1];
    NSString* interfaceName = (NSString*)[rnCommand objectAtIndex:2];
    NSString* methodName = (NSString*)[rnCommand objectAtIndex:3];
    ...
    if (sel && [self respondsToSelector:sel]) {
        ...
        NSMutableArray* args = [NSMutableArray arrayWithObject:@"message"];
        [args addObject:@"method not implemented"];
        [command sendFailureResult:args];
    }
}
    ...
    @synchronized(self) {
        ...
        if (command) {
            if (command.isSubscription) {
                [module removeSupportedEvent:commandId];
            }
...
- (void) webAppSession_connect:(JSCommand*)command
{
    ...
    WebAppSessionWrapper* wrapper = [command.module getObjectWrapper:objectId];
    WebAppSession* session = wrapper.session;
    [module addSupportedEvent:command.commandId];
    [wrapper setCallbackId:command.commandId];
...
- (void) webAppSession_closeLaunchSession:(JSCommand*)command
{
...
- (void) powerControl_powerOff:(JSCommand*)command
{
    WebOSTVService* service = (WebOSTVService*)[device serviceWithName:kConnectSDKWebOSTVServiceId];
    if (service) {
...
- (id<MediaControl>) getMediaControl:(JSCommand*)command
{
    ...
    if (objectId) {
        MediaControlWrapper* wrapper = [command.module getObjectWrapper:objectId];
...
- (id<PlayListControl>) playlistControlFromCommand:(JSCommand*)command
{
    NSString* objectId = command.args[@"objectId"];
    if (objectId) {
        PlaylistControlWrapper *wrapper = [command.module getObjectWrapper:objectId];
        if (wrapper) {
