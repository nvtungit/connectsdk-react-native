...
const eventEmitter = new NativeEventEmitter(ConnectSDK);

function execute (action, args, success, error) {
    console.log('execute ' + args);
  ConnectSDK.execute(action, args, success, error);
}
...
/** @lends DevicePicker.prototype */
{
     ...
     */
    close: function () {
        ConnectSDK.execute("closeDevicePicker", JSON.stringify([]), NULL, NULL);
    }
});
...
/** @lends DiscoveryManager.prototype */
{
    ...
    _setPairingLevel: function (pairingLevel, updateNow) {
        ...
        if (updateNow) {
            ConnectSDK.execute("setDiscoveryConfig", JSON.stringify([{pairingLevel: this._config.pairingLevel}]), NULL, NULL);
        }
    },
    ...
    _setAirPlayServiceMode: function (mode, updateNow) {
        ...
        if (updateNow) {
            ConnectSDK.execute("setDiscoveryConfig", JSON.stringify([{airPlayServiceMode: this._config.airPlayServiceMode}]), NULL, NULL);
        }
    },
    ...
    _setCapabilityFilters: function (filters, updateNow) {
        ...
        if (updateNow && this._started) {
            ConnectSDK.execute("setDiscoveryConfig", JSON.stringify([{capabilityFilters: this._config.capabilityFilters}]), NULL, NULL);
        }
    },
    ...
    startDiscovery: function (config) {
        ...
        ConnectSDK.execute("startDiscovery", JSON.stringify([this._config]), this._handleDiscoveryUpdate, this._handleDiscoveryError);
    },
    ...
    stopDiscovery: function () {
        ConnectSDK.execute("stopDiscovery", JSON.stringify([this._config]), NULL, this._handleDiscoveryError);
    },
    ...
    pickDevice: function (options, successCallback, errorCallback) {
        ...
        ConnectSDK.execute("pickDevice", JSON.stringify([options]), success, failure);
...
/** @lends ConnectableDevice.prototype */
{
    ...
    _handleAddListener: function () {
        if (this.hasListeners() && !this._subscribedToEvents) {
            ...
            ConnectSDK.execute("setDeviceListener", JSON.stringify([this._deviceId]), this._handleUpdate.bind(this), this._handleError.bind(this));
        }
    },
    ...
    connect: function () {
        this._subscribedToEvents = true;
        ConnectSDK.execute("connectDevice", JSON.stringify([this._deviceId]), this._handleUpdate.bind(this), this._handleError.bind(this));
    },
     ...
     */
    disconnect: function () {
        ConnectSDK.execute("disconnectDevice", JSON.stringify([this._deviceId]), NULL, this._handleError.bind(this));
    },
    ...
    setPairingType: function (pairingType) {
        ConnectSDK.execute("setPairingType", JSON.stringify([this._deviceId, pairingType]), this._handleUpdate.bind(this), this._handleError.bind(this));
    },
...
/** @lends Command.prototype */
{
    ...
    _send: function (ifaceName, methodName, args) {
        ...
        ConnectSDK.execute("sendCommand", JSON.stringify([self._device._deviceId, self._commandId, ifaceName, methodName, args, self._subscribe]), success, failure);
    }
});
...
/** @lends Subscription.prototype */
{
    ...
    unsubscribe: function (subscribe) {
        if (subscribe) {
            subscribe.remove();
            ConnectSDK.execute("cancelCommand", JSON.stringify([this._device._deviceId, this._commandId]), NULL, NULL);
        }   
    }
});
...
var WrappedObject = {
    ...
    acquire: function () {
        ...
        if (!this._acquired && !this._acquireRequested) {
            ...
            // FIXME report error
            ConnectSDK.execute("acquireWrappedObject", JSON.stringify([this._objectId]), this._handleEvent.bind(this), NULL);
        }
    ...
    release: function () {
        if (this._objectId) {
            ConnectSDK.execute("releaseWrappedObject", JSON.stringify([this._objectId]), NULL, NULL);
            this._objectId = null;
        }
...
/** @lends WebAppSession.prototype */
{
     ...
     */
    close: function () {
        return this._device._sendCommand("webAppSession", "closeLaunchSession", {"launchSession": this._data.launchSession}, false);
    },
...
export { ConnectSDK, eventEmitter, discoveryManager, PairingLevel, PairingType, AirPlayServiceMode, execute, Command, Subscription, LaunchSession, DevicePicker, ConnectableDevice,  CapabilityFilter, Services, KeyCodes};
